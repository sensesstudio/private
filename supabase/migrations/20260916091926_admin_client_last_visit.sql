-- Last attendance comes from private CSV evidence, independently of live
-- Mindbody balances. No studio record IDs, staff edits or sync links change.
-- Existing admin-only RLS and grants also protect the new columns.
alter table public.admin_client_packages
  add column last_visit_date date,
  add column never_attended boolean not null default false,
  add constraint admin_client_last_visit_consistent check (not never_attended or last_visit_date is null);
create index admin_client_package_attendance_idx on public.admin_client_packages(client_id,import_id)
  where raw_data ? 'Last_visit_date';
...
begin
  ...
  return jsonb_build_object('import',case when v_import.id is not null then to_jsonb(v_import)-'source_sha256' end,
    ...
    'clients',coalesce((select jsonb_agg(to_jsonb(c) || jsonb_build_object(
      'last_visit_date',v.last_visit_date,'never_attended',coalesce(v.never_attended,false)) order by c.client_name,c.id)
