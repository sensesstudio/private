...
function OnlinePackage({item,canViewReceipt}) {
  return <Card pad={22}><div className="client-activity-head"><h3>{item.package_name} · {item.format}</h3><strong>{money(item.price_hkd)}</strong></div><p>{item.credits} session{item.credits===1?'':'s'} purchased · {status(item.payment_status)}</p><p className="client-activity-meta">Purchased {when(item.paid_at)}</p><p>Valid for {item.validity_months} month{item.validity_months===1?'':'s'} from the first booked class.</p>{canViewReceipt && <PurchaseReceipt orderId={item.id}/>}</Card>;
}
const config={
