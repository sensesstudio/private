...
import { TEACHERS, BOOKINGS, CLIENTS, LOCATIONS, APPLICANTS, REVENUE, PROGRESS_LOG, GOALS, SCHEDULES } from '../../data.js';
...
export const ADMIN_NAV = [['layout-dashboard', 'Dashboard'], ['users-round', 'Clients'], ['flower-2', 'Teachers'], ['user-check', 'Approvals'], ['user-search', 'Prospects'], ['calendar-clock', 'Booking in Progress'], ['calendar', 'Confirmed Bookings'], ['wallet', 'Payments'], ['users-round', 'Team accounts']];
...
function clientAbout(c, ov) {
  ...
  return {
    ...
    injury: ov && ov.injury && ov.injury.length ? ov.injury.join(', ') : null,
    schedule: ov && ov.schedule && ov.schedule.length ? ov.schedule.map(id => (SCHEDULES.find(s => s.id === id) || {}).label || id).join(', ') : null,
    studios: ov && ov.location && ov.location.length ? (ov.location.includes('any') ? 'No preference · all studios' : ov.location.map(id => locName(id)).filter(Boolean).join(', ')) : null,
    ...
    needs: [
      ov && ov.rehabDetail ? ['Injury', ov.rehabDetail] : null,
      ov && ov.strengthDetail ? ['Strength focus', ov.strengthDetail] : null,
      ov && ov.postureDetail ? ['Posture', ov.postureDetail] : null,
    ].filter(Boolean),
            ...
            <div className="screen-scroll" style={{ maxHeight: '52vh', overflowY: 'auto', padding: '18px 24px 26px' }}>
              {view === 'About' ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {[['target', 'Primary goal', about.goal], ['user', 'Age range', about.age], ['gauge', 'Experience', about.level], ['flower-2', 'Pregnant', about.pregnant], ['stethoscope', 'Recent surgery', about.surgery], ...(about.doctor ? [['heart-pulse', 'Doctor cleared to exercise', about.doctor]] : []), ...(about.injury ? [['shield-alert', 'Injuries / sensitivities', about.injury]] : []), ...about.needs.map(([label, v]) => ['target', label, v]), ...(about.languages ? [['languages', 'Preferred language', about.languages]] : []), ...(about.schedule ? [['clock', 'Preferred times', about.schedule]] : []), ...(about.studios ? [['map-pin', 'Preferred studios', about.studios]] : []), ...(about.contact ? [['phone', 'Contact', about.contact]] : []), ...(about.channels ? [['bell', 'Notify by', about.channels]] : []), ...(about.credits ? [['ticket', 'Credits', about.credits]] : []), ...(about.billing ? [['banknote', 'Payments', about.billing]] : []), ['shield-check', 'Liability waiver', about.waiver]].map(([ic, k, v]) => (
