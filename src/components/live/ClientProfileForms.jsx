...
function WaiverEmailStatus({signatureId}) {
 const [state,setState]=useState(null);
 useEffect(()=>{let active=true;const refresh=()=>supabase.rpc('my_waiver_email_status',{p_signature_id:signatureId}).then(({data,error})=>{if(active)setState(error?{status:'unavailable'}:data);}).catch(()=>{if(active)setState({status:'unavailable'});});refresh();const timer=setInterval(refresh,15000);return()=>{active=false;clearInterval(timer);};},[signatureId]);
 const message=state?.status==='sent'?'Your signed waiver email has been accepted by the email provider. Please check your inbox and spam folder.':state?.status==='not_connected'?'Your waiver is saved. The confirmation email is queued while the studio connects email delivery.':state?.status==='queued'||state?.status==='sending'?'Your signed waiver confirmation email is queued.':state?.status==='needs_review'?'Your waiver is saved, but email delivery needs studio assistance.':state?.status==='unavailable'?'Email status is temporarily unavailable. Your signed waiver is saved.':'Your waiver is saved. No email confirmation is recorded for this signature.';
 return <p className="profile-source" role="status">{message}</p>;
}
