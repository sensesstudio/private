...
export function ClientWaiverForm({record}) {
  ...
  return <div className="profile-stack">
    <Card><h2>{signed?'Waiver signed':'Please read and sign your waiver'}</h2>{signed ? <><p>Signed by {signed.signed_name} · {profileInstant(signed.signed_at)}</p><p>Participant: {signed.participant_name} · {signed.signer_capacity==='parent_guardian'?'Parent / legal guardian':'Participant'}</p><SignatureImage signatureId={signed.id} alt="Your signature"/>{signed.copy_email_status==='sent' ? <p className="profile-source">A copy was emailed to {record.data.contact.email}.</p> : ['queued','sending'].includes(signed.copy_email_status) ? <p className="profile-source">A copy is on its way to {record.data.contact.email}.</p> : null}</> : <p>Your signed waiver is saved with your client record and a copy is emailed to {record.data.contact.email}.</p>}<p className="profile-source">Version {document.version}</p></Card>
