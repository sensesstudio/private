...
export function StudioLocations() {
  ...
  return <><p>Find your Senses studio in Hong Kong.</p><div className="profile-stack">{studios.map(l=><Card key={l.id} pad={22}><div className="profile-next"><span className="profile-icon"><Icon n="map-pin"/></span><div><h2>{l.name}</h2></div></div><p>{l.address || 'Address not recorded'}</p>{l.address_zh && <p lang="zh-Hant">{l.address_zh}</p>}{l.address && <a className="profile-text-link" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent('Senses Studio '+l.name+' '+l.address)}`} target="_blank" rel="noopener noreferrer">View map <Icon n="arrow-up-right" size={14}/></a>}</Card>)}</div>{!studios.length && <EmptyPanel icon="map-pin" title={state.loading ? 'Loading studios…' : 'Studio locations unavailable'} action={contact}>Please contact the studio for directions.</EmptyPanel>}</>;
}
