...
test('admin clients show complete CSV packages, search, pagination and private failure states', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.admin-clients-table tbody tr').first().locator('td').nth(4)).toHaveText('7');
  await expect(page.locator('.admin-clients-table tbody tr').last().locator('td').nth(4)).toHaveText('—');
