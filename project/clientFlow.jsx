// ════════════════════════════════════════════════════════════════
// Senses Pilates — CLIENT portal (booking · payment · bookings · profile · shell)
// ════════════════════════════════════════════════════════════════

// ---- BOOKING + PAYMENT FLOW (bottom sheet) ----
function BookingFlow({ t, day, slot, credits = 0, onClose, onConfirmed }) {
  const [stage, setStage] = React.useState('review'); // review · pay · processing · done
  const [pkg, setPkg] = React.useState('studio');
  const [method, setMethod] = React.useState('apple');
  const [format, setFormat] = React.useState('1:1');
  const [mode, setMode] = React.useState(credits > 0 ? 'credit' : 'buy'); // credit · buy
  ...
  const [held, setHeld] = React.useState(600);
  React.useEffect(() => { const i = setInterval(() => setHeld(h => h > 0 ? h - 1 : 0), 1000); return () => clearInterval(i); }, []);
    ...
    <div style={{ padding: '4px 22px 30px' }}>
      ...
      {stage === 'pay' && (
        <React.Fragment>
          ...
          <Button variant="dark" full size="lg" onClick={pay} icon={method === 'apple' ? 'apple' : 'lock'}>{method === 'apple' ? `Pay ${hkd(amount)}` : `Pay ${hkd(amount)}`}</Button>
        </React.Fragment>
      )}

      {stage === 'processing' && (
        <div style={{ textAlign: 'center', padding: '50px 0 40px' }}>
          <div className="spin-slow" style={{ width: 56, height: 56, borderRadius: '50%', border: '3px solid var(--sand)', borderTopColor: 'var(--accent)', margin: '0 auto 22px' }}></div>
...
const sheetTitle = { fontFamily: 'var(--font-serif)', fontWeight: 600, fontSize: 24, color: 'var(--espresso)', margin: '4px 0 18px' };
const linkBtn = { display: 'block', width: '100%', marginTop: 12, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 12.5, color: 'var(--taupe)', textAlign: 'center', textDecoration: 'underline', textUnderlineOffset: 3 };
    ...
    <div style={{ padding: '4px 24px 36px', textAlign: 'center' }}>
      {!done ? (
        <React.Fragment>
          ...
          <textarea placeholder="Add a few words (optional)" style={{ ...inputStyle, minHeight: 88, resize: 'none', fontFamily: 'var(--font-sans)' }}></textarea>
    ...
    <div style={{ padding: '8px 20px 28px' }}>
      ...
      <div style={{ borderRadius: 22, background: 'var(--espresso)', color: 'var(--cream)', padding: '22px 22px 20px', position: 'relative', overflow: 'hidden', marginBottom: 18 }}>
        ...
        <div style={{ height: 6, background: 'rgba(250,247,243,.2)', borderRadius: 999, overflow: 'hidden', margin: '12px 0 4px' }}>
          <div style={{ width: pct + '%', height: '100%', background: 'var(--accent)', borderRadius: 999 }}></div>
        </div>
      ...
      <div style={{ background: 'var(--ivory)', borderRadius: 20, overflow: 'hidden', border: '1px solid var(--border-soft)' }}>
        {[['user-round', 'About me', onRestart], ['clipboard-list', 'Progress log', () => setShowLog(true)], ['credit-card', 'Payment & packages'], ['bell', 'Notifications'], ['heart', 'Saved instructors'], ['settings', 'Preferences'], ['log-out', 'Sign out']].map(([ic, l, fn], i, a) => (
...
function ProgressLog({ onClose }) {
  const live = useLiveProgress('c1'); // Mara
      ...
      <div className="screen-scroll" style={{ flex: 1, minHeight: 0, padding: '16px 18px 28px' }}>
        <div style={{ position: 'relative', paddingLeft: 26 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {merged.map((p, idx) => { const t = teacherById(p.tId); const num = totalN - idx; const isNew = !!p.id && p.id.startsWith('live'); return (
...
// ════════════════════════════════════════════════════════════════
// CLIENT PORTAL — state machine
// ════════════════════════════════════════════════════════════════
function ClientPortal() {
  const [stage, setStage] = React.useState('browse'); // browse · login · intake · app
  const [tab, setTab] = React.useState('Home');
  const [answers, setAnswers] = React.useState({ age: '25–34', goals: ['rehab', 'posture'], level: 'some', injury: ['Lower back'], schedule: ['am'], location: 'central' });
  const [detail, setDetail] = React.useState(null);     // teacher
  const [booking, setBooking] = React.useState(null);   // {t, day, slot}
  const [rating, setRating] = React.useState(null);     // teacher
  const [searchLoading, setSearchLoading] = React.useState(false);
  const [extraBookings, setExtraBookings] = React.useState([]);
  ...
  // browse (public) & login & intake are full-screen (no nav)
  if (stage === 'browse') {
    return (
      <PhoneFrame>
  ...
  return (
    <PhoneFrame navBar={<ClientNav tab={tab} setTab={setTab} />}>
      ...
      {/* Teacher detail — full-screen overlay inside phone */}
      {detail && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 70, background: 'var(--cream)', overflow: 'auto' }} className="screen-scroll">
...
Object.assign(window, { BookingFlow, ClientBookings, RatingSheet, ClientProfile, ClientPricing, ClientPortal });
