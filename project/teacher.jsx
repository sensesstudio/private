// ════════════════════════════════════════════════════════════════
// Senses Pilates — TEACHER portal
// ════════════════════════════════════════════════════════════════

// reusable responsive workspace: sidebar on desktop, bottom nav on mobile
function Workspace({ title, nav, tab, setTab, accent, children, headRight }) {
  const { mobile } = useVP();
  if (mobile) {
    return (
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', background: 'var(--cream)' }}>
        ...
        <div style={{ flex: 'none', background: 'rgba(250,247,243,.95)', backdropFilter: 'blur(14px)', borderTop: '1px solid var(--border)', padding: '9px 12px calc(9px + env(safe-area-inset-bottom))', display: 'flex', justifyContent: 'space-around' }}>
          {nav.map(([ic, label]) => { const on = tab === label; return (
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={d.m} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, height: '100%', justifyContent: 'flex-end' }}>
            ...
            <div style={{ width: '100%', maxWidth: 42, height: h, borderRadius: 8, background: on ? 'var(--accent)' : 'var(--sand)', transition: 'height .6s var(--ease)' }}></div>
...
function TeacherPortal() {
  ...
  const avatarBtn = <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}><Avatar t={me} size={34} /><span style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 12.5, color: 'var(--espresso)' }} className="hide-mobile">{me.name}</span></div>;
    ...
    <div style={{ minHeight: '100vh', display: 'flex', background: 'var(--cream)' }}>
      {!mobile && (
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
          <image-slot id="teacher-login-hero" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }} shape="rect" fit="cover" placeholder="Drop a studio photo"></image-slot>
          <div className="app-ph taupe" style={{ position: 'absolute', inset: 0, zIndex: -1 }}></div>
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(58,50,44,.35), rgba(58,50,44,.65))' }}></div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {today.map((b, i) => { const c = CLIENTS.find(x => x.id === b.cId); return (
              <div key={b.id} style={{ display: 'flex', gap: 14, alignItems: 'center', padding: '14px 0', borderBottom: i < today.length - 1 ? '1px solid var(--border-soft)' : 'none' }}>
                ...
                <Pill color="var(--accent)" bg="var(--accent-tint)">{locName(b.locId)}</Pill>
              </div>
            ); })}
          </div>
        </Card>
...
function TeacherAvailability({ me }) {
  ...
  // availability keyed per location → each studio holds its own week
  const [loc, setLoc] = React.useState(me.locId);
  const [grids, setGrids] = React.useState(() => {
    const all = {};
    LOCATIONS.forEach((L, li) => {
      const g = {};
      // teacher's home studio starts with some open slots; others start empty
  ...
  return (
    <div style={{ padding: mobile ? '20px 16px 28px' : '34px 40px 40px', maxWidth: 1000, margin: '0 auto' }}>
      <PageHead eyebrow="Your week" title="Availability" sub="Choose a studio, then tap slots to open or close them. Clients book only what you open." right={<Pill color="var(--accent)" bg="var(--accent-tint)">{openCount} open · {locName(loc)}</Pill>} />
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 16, fontFamily: 'var(--font-sans)', fontWeight: 300, fontSize: 12.5, color: 'var(--fg3)' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7 }}><span style={{ width: 14, height: 14, borderRadius: 4, background: 'var(--accent)' }}></span> Open</span>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7 }}><span style={{ width: 14, height: 14, borderRadius: 4, background: 'var(--ivory)', border: '1px solid var(--border)' }}></span> Closed</span>
      </div>
    </div>
  );
}
...
function TeacherSessions({ me }) {
  ...
  const [logFor, setLogFor] = React.useState(null); // {b, c}
            ...
            <Card key={b.id} pad={14}>
              <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  {b.status === 'completed' ? <Pill color="var(--taupe)" bg="var(--sand)" style={{ marginTop: 4 }}>Completed</Pill> : b.status === 'cancelled' ? <Pill color="var(--terracotta)" bg="rgba(185,117,91,.14)" style={{ marginTop: 4 }}>Cancelled</Pill> : <Pill color="var(--accent)" bg="var(--accent-tint)" style={{ marginTop: 4 }}>Confirmed</Pill>}
                </div>
              </div>
              {b.status !== 'cancelled' && (
        ...
        {logFor && (
          <div style={{ padding: 26 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              ...
              <div><h2 style={{ fontFamily: 'var(--font-serif)', fontWeight: 600, fontSize: 20, color: 'var(--espresso)', margin: 0 }}>Log progress</h2><div style={{ fontFamily: 'var(--font-sans)', fontWeight: 300, fontSize: 12.5, color: 'var(--fg3)' }}>{logFor.c.name} · {logFor.c.goal}</div></div>
            </div>
            ...
            <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Focus, wins, form cues, what to progress next time…" style={{ ...inputStyle, minHeight: 110, resize: 'none', marginTop: 8 }}></textarea>
      ...
      <Card pad={0}>
        ...
        {completed.slice(0, 5).map((b, i, a) => { const c = CLIENTS.find(x => x.id === b.cId); return (
          <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '14px 20px', borderBottom: i < a.length - 1 ? '1px solid var(--border-soft)' : 'none' }}>
            <Avatar t={c} size={38} />
            <div style={{ flex: 1 }}><div style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 13.5, color: 'var(--espresso)' }}>{c.name}</div><div style={{ fontFamily: 'var(--font-sans)', fontWeight: 300, fontSize: 11.5, color: 'var(--fg3)' }}>{new Date(b.date + 'T00:00:00').toLocaleDateString('en-HK', { day: 'numeric', month: 'short' })} · {b.type}</div></div>
...
function TeacherProfile({ me, onLogout }) {
  const { mobile } = useVP();
  const [rate, setRate] = React.useState(me.rate);
  return (
    <div style={{ padding: mobile ? '20px 18px 28px' : '34px 40px 40px', maxWidth: 760, margin: '0 auto' }}>
      ...
      <Card pad={0} style={{ overflow: 'hidden', marginBottom: 18 }}>
        <div style={{ position: 'relative', height: 150 }}>
          <image-slot id="teacher-self-photo" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }} shape="rect" fit="cover" placeholder="Drop your profile photo"></image-slot>
          <div className={'app-ph ' + me.ph} style={{ position: 'absolute', inset: 0, zIndex: -1 }}></div>
        </div>
      ...
      <Card pad={22} style={{ marginBottom: 16 }}>
        ...
        <textarea defaultValue={me.bio} style={{ ...inputStyle, minHeight: 90, resize: 'none', marginTop: 8 }}></textarea>
      </Card>
...
Object.assign(window, { Workspace, PageHead, Stat, BarChart, TeacherPortal });
