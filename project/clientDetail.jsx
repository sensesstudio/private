// ════════════════════════════════════════════════════════════════
// Senses Pilates — CLIENT portal (home · search · detail · booking · profile)
// ════════════════════════════════════════════════════════════════

const sortByMatch = (arr) => [...arr].sort((a, b) => b.match - a.match);

// ---- HOME (after intake) ----
function ClientHome({ onOpen, goSearch, answers }) {
  const ranked = sortByMatch(TEACHERS);
  const top = ranked[0];
  const goalLabels = (answers.goals || []).map(g => (GOALS.find(x => x.id === g) || {}).label).filter(Boolean);
  // synthesise from the instructor progress log (focus areas + comments), not just session count
  const log = (typeof PROGRESS_LOG !== 'undefined' ? PROGRESS_LOG : []);
  const latest = log[0];
  ...
  return (
    <div style={{ padding: '8px 20px 28px' }}>
      ...
      {/* AI progress summary — synthesises past visits + browsing into a nudge */}
      <div style={{ marginTop: 22, borderRadius: 24, overflow: 'hidden', position: 'relative', background: 'var(--espresso)' }}>
        <image-slot id="client-home-hero" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: 0.42 }} shape="rect" fit="cover" placeholder=""></image-slot>
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(160deg, rgba(58,50,44,.62), rgba(58,50,44,.86))' }}></div>
        <div style={{ position: 'relative', padding: '20px 20px 18px', color: 'var(--cream)' }}>
          ...
          <p style={{ fontFamily: 'var(--font-serif)', fontWeight: 500, fontSize: 19, lineHeight: 1.4, color: 'var(--cream)', margin: '14px 0 4px' }}>
            Across <span style={{ color: 'var(--blush)' }}>28 sessions</span>, {logTeacher.name.split(' ')[0]} has guided you through <span style={{ color: 'var(--blush)' }}>{(recentFocus[0] || 'lower-back mobility').toLowerCase()}</span> — and her latest note says it’s paying off. You’re <span style={{ color: 'var(--blush)' }}>2 sessions</span> from your next milestone.
          </p>

          {/* latest instructor comment — pulled from the progress log */}
          {latest &&
          <div style={{ display: 'flex', gap: 10, background: 'rgba(250,247,243,.1)', borderRadius: 14, padding: '12px 13px', margin: '14px 0 0' }}>
          ...
          <div style={{ margin: '14px 0 4px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 11, opacity: .85, marginBottom: 6 }}>
              <span>30-session milestone</span><span style={{ whiteSpace: 'nowrap' }}>28 / 30</span>
            </div>
            <div style={{ height: 7, background: 'rgba(250,247,243,.2)', borderRadius: 999, overflow: 'hidden' }}>
              <div style={{ width: '93%', height: '100%', background: 'var(--blush)', borderRadius: 999 }}></div>
            </div>
          </div>
          ...
          {/* reminder nudge — echoes the instructor's next-time plan */}
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 9, background: 'rgba(250,247,243,.1)', borderRadius: 14, padding: '11px 13px', margin: '14px 0 14px' }}>
            <Icon n="bell" size={15} color="var(--blush)" style={{ marginTop: 1, flex: 'none' }} />
            <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 300, fontSize: 12.5, lineHeight: 1.5, color: 'var(--cream)' }}>{logTeacher.name.split(' ')[0]} planned <span style={{ color: 'var(--blush)' }}>single-leg work</span> for your next session — book this week to keep the momentum while it’s fresh.</span>
          </div>
          ...
          {ranked.filter(t => t.online).map(t => (
            <div key={t.id} className="tap" onClick={() => onOpen(t)} style={{ flex: 'none', width: 156, background: 'var(--ivory)', border: '1px solid var(--border-soft)', borderRadius: 18, padding: 14, boxShadow: 'var(--shadow-sm)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: 9, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--accent)' }}><span className="live-dot"></span>Free</span>
              </div>
            ...
            <div style={{ display: 'flex', gap: 13, padding: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                  ...
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, flex: 'none', fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: 9, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--accent)' }}><span className="live-dot"></span>{t.soon.replace('Free ', '')}</span>
                </div>
...
// personalised "why this instructor" rationale — tied to her goals/history/profile
...
// ---- SEARCH / all matches ----
function ClientSearch({ onOpen, loading }) {
  const { mobile } = useVP();
...
function TeacherSkel() {
  return (
    <div style={{ background: 'var(--ivory)', borderRadius: 22, padding: 16, display: 'flex', gap: 14, border: '1px solid var(--border-soft)' }}>
      <Skel w={64} h={64} r={18} />
      <div style={{ flex: 1 }}>
        <Skel w="60%" h={18} /><div style={{ height: 8 }}></div>
        <Skel w="80%" h={12} /><div style={{ height: 14 }}></div>
        <Skel w="40%" h={12} />
      </div>
    </div>
  );
...
// ---- TEACHER DETAIL ----
function TeacherDetail({ t, onClose, onBook }) {
  const { mobile } = useVP();
  const [avail, setAvail] = React.useState(() => buildAvail(t, 0));
  ...
  const Header = (
    <div style={{ position: 'relative' }}>
      <div style={{ position: 'relative', height: mobile ? 230 : 240 }}>
        <image-slot id={'teacher-photo-' + t.id} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }} shape="rect" fit="cover" placeholder="Drop instructor photo"></image-slot>
        <div className={'app-ph ' + (t.ph || '')} style={{ position: 'absolute', inset: 0, zIndex: -1 }}></div>
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(58,50,44,.3) 0%, transparent 35%, rgba(58,50,44,.55) 100%)' }}></div>
    ...
    <div style={{ minHeight: '100%', background: 'var(--cream)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: mobile ? '18px 20px 16px' : '22px 26px 16px', maxWidth: 640, width: '100%', margin: '0 auto', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <span style={{ ...metaItem, color: 'var(--accent)', fontWeight: 500 }}><span className="live-dot"></span> {t.responds}</span>
        </div>
        ...
        <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', marginBottom: 8 }}>
          <div style={{ flex: 1, minWidth: 150 }}><div style={labelMini}>Certifications</div>
          ...
          <div style={{ minWidth: 120 }}><div style={labelMini}>Languages</div>
        ...
        <div style={{ marginTop: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 11, color: 'var(--accent)' }}><span className="live-dot"></span> Live · updates in real time</span>
          </div>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', margin: '0 -2px 14px', padding: '2px', scrollbarWidth: 'none' }} className="screen-scroll">
            {avail.map((dd, i) => {
              ...
              return <button key={i} className="tap" onClick={() => setDay(i)} style={{ flex: 'none', width: 52, textAlign: 'center', padding: '10px 0', borderRadius: 16, cursor: 'pointer', border: '1px solid ' + (on ? 'var(--accent)' : 'var(--border)'), background: on ? 'var(--accent)' : 'var(--ivory)' }}>
                ...
                <div style={{ width: 5, height: 5, borderRadius: 999, margin: '0 auto', background: free > 0 ? (on ? '#fff' : 'var(--accent)') : 'transparent' }}></div>
              </button>;
            })}
          </div>
...
const metaItem = { display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: 'var(--font-sans)', fontWeight: 300, fontSize: 13, color: 'var(--taupe)' };
const labelMini = { fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: 10, letterSpacing: '.16em', textTransform: 'uppercase', color: 'var(--fg3)', marginBottom: 9 };

Object.assign(window, { ClientHome, ClientSearch, TeacherDetail, TeacherSkel, EmptyState, sortByMatch });
